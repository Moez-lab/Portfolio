...
const Footer = () => {
  return (
    <footer className="w-full relative z-0 mt-20 bg-slate-100 dark:bg-primary border-t border-slate-200 dark:border-white/5 transition-colors duration-300">
      ...
      <div className="sm:px-16 px-6 py-10 flex flex-col items-center justify-center max-w-7xl mx-auto">
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center md:items-start gap-10 md:gap-0">
          ...
          >
            ...
            <p className="text-slate-600 dark:text-secondary text-[14px] mt-2 max-w-[250px] text-center md:text-left leading-relaxed">
          ...
          <div className="flex flex-col items-center md:items-start">
            <h3 className="text-slate-900 dark:text-white font-bold text-[18px] mb-4">Quick Links</h3>
            <ul className="flex flex-col gap-3 items-center md:items-start">
              {navLinks.map((link) => (
                <li key={link.id}>
                  <a
                    href={`#${link.id}`}
                    className="text-slate-600 dark:text-secondary hover:text-[#915EFF] text-[15px] font-medium transition-colors duration-300 relative group"
                  >
                    {link.title}
        ...
        {/* Bottom Section */}
        <div className="w-full mt-12 pt-6 border-t border-slate-300/80 dark:border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0">
          <p className="text-slate-600 dark:text-secondary text-[14px] font-medium text-center md:text-left">
            &copy; {new Date().getFullYear()} Moez | All rights reserved.
          </p>
          <div className="flex gap-6">
            <a href="#" className="text-slate-600 dark:text-secondary text-[14px] font-medium hover:text-slate-900 dark:hover:text-white transition-colors duration-300 cursor-pointer">
              Privacy Policy
            </a>
            <a href="#" className="text-slate-600 dark:text-secondary text-[14px] font-medium hover:text-slate-900 dark:hover:text-white transition-colors duration-300 cursor-pointer">
              Terms of Service
